...
const cargoData = {
  ...
  "metodo_pago": "tarjeta",
  "tarjeta": {
    "token": "c47c8cfa-c09a-48e8-89fb-e2c9c45f62a5" // Reemplaza con el token real de la tarjeta
  },
...
.then(response => {
  const logMessage = `Cargo Rechazado:\nRespuesta: ${JSON.stringify(response.data.data.cargo.procesador, null, 2)}\n\n`;
  console.log(logMessage);  // También imprimir en consola

  // Escribir el log en un archivo separado
  fs.appendFile('respueta.txt', logMessage, (err) => {
    if (err) {
...
.catch(error => {
  ...
  fs.appendFile('log_cargo_autorizado.txt', errorMessage, (err) => {
    if (err) {
      console.error('Error al escribir en el archivo log_cargo_autorizado.txt:', err);
    } else {
